 ...
 * 返回值: 成功返回0，失败返回负的错误码
 */
int save_pgm_to_file(const char *filename, uint8_t *data, unsigned int width, unsigned int height, size_t size) {
    struct file *file;
    loff_t pos = 0;
    mm_segment_t old_fs;
    char *header;
    ...
    // 写入图像数据
    ret = vfs_write(file, (const char __user *)data, size, &pos);
    if (ret < 0) {
